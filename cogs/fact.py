    ...
    async def fact(self,ctx):
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://uselessfacts.jsph.pl/random.json?language=en') as r:
                res = await r.json()
                text = res["text"]
                m = discord.Embed(title = 'Fun fact' ,description=f"{text}" , color = ctx.author.color)
    ...
    async def dog(self,ctx):
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://dog-api.kinduff.com/api/facts?number=1') as r:
                res = await r.json()
                title = res["facts"][0]
                m = discord.Embed(title = 'Here is a Woofy Fact ...' ,description=f"{title}" , color = ctx.author.color)
                await ctx.send(embed= m)
    ...
    async def cat(self,ctx):
        async with aiohttp.ClientSession() as cs:
            async with cs.get('https://catfact.ninja/facts?limit=1') as r:
                ...
                m = discord.Embed(title = 'Cat fact' ,description=f"{title}" , color = ctx.author.color)
                await ctx.send(embed= m)
